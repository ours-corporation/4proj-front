    ...
    return (
        <div className="flex flex-col sm:flex-row items-center gap-6 xl:gap-10 w-full">
            ...
            <div className="relative flex-shrink-0 w-36 h-36 xl:w-44 xl:h-44">
                <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                    ...
                    <circle
                        ...
                        fill="none"
                        className="stroke-black/[0.06] dark:stroke-white/[0.05]"
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-xl font-bold text-txt-primary dark:text-[#ededed]">{usedPct}%</span>
                    <span className="text-[10px] text-txt-secondary dark:text-[#444] mt-0.5">utilisé</span>
                </div>
            </div>
            ...
            <div className="flex-1 w-full space-y-4">
                {filtered.length === 0 ? (
                    <p className="text-sm text-txt-secondary dark:text-[#444] text-center py-4">Aucune donnée</p>
                ) : filtered.map((cat) => {
                        ...
                        <div key={cat.label}>
                            <div className="flex items-center justify-between mb-1.5">
                                <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <span className="text-sm text-txt-primary dark:text-[#ccc]">{cat.label}</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className="text-xs text-txt-secondary dark:text-[#555]">{convertFileSize(cat.bytes)}</span>
                                    <span className="text-xs font-semibold text-txt-secondary dark:text-[#888] w-10 text-right">
                                        {pctOfUsed.toFixed(1)}%
                                    </span>
                                </div>
                            </div>
                            <div className="w-full h-1.5 bg-black/[0.05] dark:bg-white/[0.05] rounded-full overflow-hidden">
                                <div
                ...
                {/* Total row */}
                {filtered.length > 0 && (
                    <div className="pt-3 border-t border-border-subtle dark:border-white/[0.06] flex items-center justify-between">
                        <span className="text-xs text-txt-secondary dark:text-[#444]">Total utilisé</span>
                        <span className="text-sm font-semibold text-txt-primary dark:text-[#ededed]">{convertFileSize(usedTotal)}</span>
                    </div>
                )}
            </div>
        </div>
