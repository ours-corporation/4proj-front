...
export default function FileCard({ file, thumbnailUrl, downloadFile, editFile, shareFile, moveFile, deleteFile, copyFile, isTrash, restoreFile }: FileProps) {
    ...
    return (
        ...
        >
            {/* Thumbnail / Icon area */}
            <div className="h-[90px] bg-[#0d0d0d] flex items-center justify-center relative overflow-hidden">
                {thumbnailUrl ? (
                ...
                {menuItems.length > 0 && (
                    <div className="absolute top-2 right-2">
                        <button
                            ...
                            className="w-7 h-7 flex items-center justify-center rounded-[6px] text-[#666] hover:text-[#ededed] hover:bg-black/40 transition-colors"
                        >
            ...
            <div className="px-3 py-2.5">
                <p className="text-[13px] font-medium text-[#ccc] truncate" title={file.fullName}>{file.fullName}</p>
                <p className="text-[11px] text-[#444] mt-0.5">{convertFileSize(file.size_bytes)} · {converCreatedAt(file.createdAt)}</p>
            </div>
        </div>
    );
}
